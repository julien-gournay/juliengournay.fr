<!DOCTYPE html>
<html lang="fr">
<head>
    <?php include 'head.php'; ?>
    <title>Documents - Julien Gournay</title>

    <link rel="stylesheet" type="text/css" href="css/document.css">

    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-T6V1XWV51D"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
    
        gtag('config', 'G-T6V1XWV51D');
    </script>
</head>

<body>    
    <!-- NAVBAR -->
    <?php include 'navbar.php'; ?>

    <section id="sec-1">
        <div>
            <h1>DOCUMENTS</h1>
        </div>
        <a href="#sec-2">
            <div class="scroll-down"></div>   
        </a>
    </section>

    <section id="sec-2">
        <div>
            <form action="">
                
            </form>
        </div>
        <!--<div>
            <table>
                <thead>
                    <tr>
                        <th scope="col">Type</th>
                        <th scope="col">Nom</th>
                        <th scope="col">Description</th>
						<th scope="col">Format</th>
                        <th scope="col">Taille</th>
                        <th scope="col">Date création</th>
                        <th scope="col">Date modification</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        /*
                        // CONNEXION BDD
                        include 'php/config.php';
                    
                        if($mabase){
                            $req = "SELECT * FROM document ORDER BY datePub DESC";
                            $res = mysqli_query($cnt,$req);
                        }
                        $nb = 0;
                        While ($tab=mysqli_fetch_row($res)){
                            $titre = $tab[1];
                            $description = $tab[2];
                            $type = $tab[3];
                            $format = $tab[4];
                            $taille = $tab[5];
                            $lien = $tab[6];
                            $dpub = $tab[7];
                            $dmaj = $tab[8];
                            echo("<tr>
                                <td>");
                                    if($format == "1"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "2"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "3"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "4"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "5"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "6"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "7"){echo("<img src=\"data:image/png;base64,\">");}
                                    if($format == "8"){echo("<img src=\"data:image/png;base64,\" alt=\"image-file\">");}
                                    echo("</td>
                                <td scope=\"row\"><a href=$lien>$titre</a></td>
                                <td>$description</td>
							    <td>");
                                    switch($type) {
                                        case 0 :echo("CV"); break;
                                        case 1 :echo("Rapport"); break;
                                        case 2 :echo("Synthèse"); break;
                                    }
							    echo("</td>
                                <td>$taille</td>
                                <td>$dpub</td>
                                
                                <td>");
                                    if($dmaj != "0000-00-00"){echo("$dmaj");}
                                    if($dmaj == "0000-00-00"){echo("Aucune modification");}
                                echo("</td>
                            </tr>");
                        }*/
                    ?>
                </tbody>
            </table>
        </div>-->


        <div class="liste-doc">
            <?php
                // CONNEXION BDD
                include 'php/config.php';
            
                if($mabase){
                    $req = "SELECT * FROM document ORDER BY datePub DESC";
                    $res = mysqli_query($cnt,$req);
                }
                $nb = 0;
                While ($tab=mysqli_fetch_row($res)){
                    $titre = $tab[1];
                    $description = $tab[2];
                    $type = $tab[3];
                    $format = $tab[4];
                    $taille = $tab[5];
                    $lien = $tab[6];
                    $dpub = $tab[7];
                    $dmaj = $tab[8];
                    echo("<div class=\"card-doc\">");
                            if($format == "1"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "2"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "3"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "4"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "5"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "6"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "7"){echo("<img src=\"data:image/png;base64,\">");}
                            if($format == "8"){echo("<img src=\"data:image/png;base64,\" alt=\"image-file\">");}
                            echo("<a href=$lien>$titre</a>
                        <p>$description</p>");
                            switch($type) {
                                case 0 :echo("CV"); break;
                                case 1 :echo("Rapport"); break;
                                case 2 :echo("Synthèse"); break;
                            }
                        echo("
                        <p>$taille</p>
                        <p>Importé le $dpub</p>");
                            if($dmaj != "0000-00-00"){echo("Modifé le $dmaj");}
                        echo("</div><hr>");
                }
            ?>
        </div>
    </section>

    <!-- FOOTER -->
    <?php include 'footer.php'; ?>

</body>
</html>